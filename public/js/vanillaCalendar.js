let vanillaCalendar = {
    ...
    createDay: function (num, day, year) {
        ...
        if (num === 1) {
            // we add padding
            var datePadding = document.createElement('div')
            if (day === 0) {
                datePadding.style.marginLeft = (6 * 14.28) + '%'
            } else {
                datePadding.style.marginLeft = (day * 14.28) + '%'
            }
            this.month.appendChild(datePadding)
        }

        if ((this.options.disablePastDays && this.date.getTime() <= this.todaysDate.getTime() - 1) || (this.options.disableWeekend && this.date.getDay() >= 6)){
    ...
    dateClicked: function () {
        ...
        for (var i = 0; i < this.activeDates.length; i++) {
            this.activeDates[i].addEventListener('click', function (event) {
                let picked = document.querySelector('#date')
                picked.valueAsDate = new Date(this.dataset.calendarDate);
                document.querySelector('#fake_date').value = picked.value;
